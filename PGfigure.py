...
def PG23():
    ...
    label_dict = {}
    label_node_map = {}
    second_label_dict = {} # dictionary of labels expressed as coordinates in Z3 \cup \pm \infty
    field_coordinates = {0:'$h$',
                   1:'(0,2)',
                   2:'(1,2)',
                   3:'(2,2)',
                   4:'(0,1)',
    ...
    for vertex in vertex_coordinates.keys():
        if vertex == 0:
            circle = plt.Circle(
                        ...
                        radius=0.15,      # Radius
                        facecolor= 'black',#'white',   # Fill color
                        edgecolor= 'black', #'red',   # Outline color
        ...
        if vertex > 9 or vertex == 0:
            epsx = 0.08#0.13
        else:
            epsx = -0.01#0.07
        epsy = 0.07
        coords = np.array([vertex_coordinates[vertex][0],vertex_coordinates[vertex][1]]) - np.array([epsx, epsy])
        coords2 = np.array([vertex_coordinates[vertex][0],vertex_coordinates[vertex][1]]) - np.array([0.15, 0.07]) + np.array([epsx, epsy])
        text = TextPath((0,0), str(vertex), size = 0.2)
        text2 = TextPath((0,0), field_coordinates[vertex], size = 0.11) # text for node labels in field coordinates
        trans = Affine2D().translate(*coords2)
        """
        ...
        text_patch2 = PathPatch(text2, color='white', transform=trans + ax.transData, zorder = 30)
        text_patch.set_visible(False)
        text_patch2.set_visible(True)
        if vertex == 0:
            print('setting invisible')
            text_patch.set_visible(False)
        # this makes sure that the text patches appear in teh figure
        ax.add_patch(text_patch)
        ax.add_patch(text_patch2)
        vertex_dict[vertex] = circle
        label_dict[vertex] = text_patch
        second_label_dict[vertex] = text_patch2
